package br.com.mvj.prepag.domain.card.entities;
...
public class Card {
	...
	@Builder()
	private Card(String nome, BigDecimal saldo, PasswordEncoderPort passwordEncoder) {
		super();
		this.nome = nome;
		this.saldo = saldo;
	...
	public static CardBuilder builder(String nome, BigDecimal saldo, PasswordEncoderPort passwordEncoder) {
        return builder().nome(nome).saldo(saldo).passwordEncoder(passwordEncoder);
    }
	
	
	private void gerarCard(PasswordEncoderPort passwordEncoder) {
		gerarUUID();
		gerarValidade();
		gerarNumero();
		gerarCvv();
	...
	private void gerarSenha(PasswordEncoderPort passwordEncoder) {
